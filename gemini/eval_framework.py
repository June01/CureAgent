...
class GeminiModel(BaseModel):
    ...
    def load(self, **kwargs):
        ...
        try:
            ...
            logger.info(f"Loaded Gemini client for model: {self.model_name}")
        except ImportError as e:
            logger.error(f"Failed to import google-genai: {e}")
            logger.error("Please install with: pip install google-genai")
            raise
        except Exception as e:
            logger.error(f"Failed to load Gemini client: {e}")
            raise

    def inference(self, prompt: str, max_tokens: int = 1024, summary: bool = False) -> Tuple[str, list]:
        """Gemini inference with optional Google Search"""
        search_enabled = self.google_search_enabled
        ...
        try:
            ...
            if summary:
                ...
                # Generate enhanced_prompt based on search_enabled setting
                if search_enabled:
                    # Enhanced prompt with Google Search instructions
                    enhanced_prompt = (
                        "You are given a multi-turn conversation in the following format:\n"
                        "[{role: user, content: ...}, {role: assistant, content: ..., tools: [...]}, {role: tool, content: ...}, ...]\n\n"
                        "Please answer the user's request based on the conversation above.\n"
                        "Instructions:\n"
                        "1. If the information in the previous conversation is sufficient, directly provide the final answer.\n"
                        "2. If the information is insufficient, use Google Search to obtain more up-to-date and relevant medical information (such as latest guidelines, literature, etc.), and then provide the answer.\n"
                        "3. The final output should start with step-by-step reasoning in detail, and end with [FinalAnswer] as the conclusion marker.\n"
                        "4. For multiple-choice questions, the output format should be:\n"
                        "   Reasoning text \n"
                        "   [FinalAnswer]\n"
                        "   Answer: (A/B/C/D)\n"
                        "   Reason: Please explain your choice in summary.\n"
                        "5. For open-ended questions, provide a complete answer after [FinalAnswer].\n"
                        "Make sure your answer is accurate, evidence-based, and cite information from Google Search if used.\n\n"
                        f"Conversation:\n{prompt}\n"
                    )
                else:
                    # Enhanced prompt WITHOUT Google Search instructions (when search is disabled)
                    enhanced_prompt = (
                        "You are given a multi-turn conversation in the following format:\n"
                        "[{role: user, content: ...}, {role: assistant, content: ..., tools: [...]}, {role: tool, content: ...}, ...]\n\n"
                        "Please answer the user's request based on the conversation above.\n"
                        "Instructions:\n"
                        "1. If the information in the previous conversation is sufficient, directly provide the final answer.\n"
                        "2. Use your knowledge to provide the answer based on the conversation.\n"
                        "3. The final output should start with step-by-step reasoning in detail, and end with [FinalAnswer] as the conclusion marker.\n"
                        "4. For multiple-choice questions, the output format should be:\n"
                        "   Reasoning text \n"
                        "   [FinalAnswer]\n"
                        ...
                        "   Reason: Please explain your choice in summary.\n"
                        "5. For open-ended questions, provide a complete answer after [FinalAnswer].\n"
                        "Make sure your answer is accurate and evidence-based.\n\n"
                        f"Conversation:\n{prompt}\n"
                    )

            # Prepare prompt and config based on Google Search setting
            if search_enabled:
                # Enhanced prompt to encourage Google Search usage
                # Construct enhanced_prompt. The input 'prompt' is a conversation message list in the format:
                # [{role: user, content: xx}, {role: assistant, content: xxxx, tools:[xx]}, {role: tool, content: xxxx}]
        
                # Generate response using Gemini with Google Search
                response = self.client.models.generate_content(model=model_name, contents=enhanced_prompt,config={"tools": [{"google_search": {}}]},)
                print("Search is Enabled")
            else:
                # Use original prompt without search enhancement
                response = self.client.models.generate_content(
                    model=model_name,
                    contents=enhanced_prompt,
                    # No tools config - no Google Search
                )
            ...
            # Display response in Markdown format to terminal
            self._display_markdown_response(response_text)

            # Log detailed search information if available
            if search_enabled:
                if hasattr(response, "candidates") and response.candidates:
                    ...
                    if (
                        ...
                        if (
                            ...
                            if search_pages:
                                logger.info(
                                    f"🌐 Search pages used: {', '.join(search_pages)}"
                                )

                        # Log search entry point if available
                        if (
                            hasattr(candidate.grounding_metadata, "search_entry_point")
                            and candidate.grounding_metadata.search_entry_point
                        ):
                            logger.info("🔗 Search entry point HTML available")
                    else:
                        logger.warning(
                            "⚠️ No grounding metadata found - Google Search may not have been used"
                        )
            else:
    ...
    def _display_markdown_response(self, response_text: str):
        """Display response in Markdown format to terminal"""
        try:
            # Try to use rich for better markdown rendering
            try:
                ...
                # Fallback to simple text display with markdown indicators
                print("\n" + "=" * 80)
                print("📄 GEMINI RESPONSE (Markdown)")
                print("=" * 80)
                print(response_text)
                print("=" * 80 + "\n")
        except Exception as e:
            logger.warning(f"Failed to display markdown response: {e}")
...
class GPTOSS20BModel(BaseModel):
    ...
    def inference(
        ...
        try:
            parsed = self.enc.parse_messages_from_completion_tokens(
                gen_tokens, role=Role.ASSISTANT
            )
...
class CompetitionKit:
    ...
    def load_model(self, model_name: str, model_type: str = "auto", **kwargs):
        ...
        Args:
            model_name: Name/path of the model (e.g., "gpt-4o-mini", "meta-llama/Llama-2-7b-chat-hf", "gemini-pro")
            model_type: Type of model ("chatgpt", "gemini", "local", "custom", "auto" for auto-detection)
        ...
        if model_type == "chatgpt":
            ...
            google_search_enabled = kwargs.get("google_search_enabled", True)
            self.model = GeminiModel(
                model_name, api_key=api_key, google_search_enabled=google_search_enabled
            )
    ...
    def _detect_model_type(self, model_name: str) -> str:
        ...
        if "gpt-oss-20b" in model_name.lower():
            return "gpt-oss-20b"
        if any(name in model_name.lower() for name in ["gemini", "bard"]):
            return "gemini"
        if any(
    ...
    def _load_dataset(self, dataset_config: Dict) -> List[Dict]:
        ...
        if not data_path:
            raise ValueError("data_path not specified in dataset configuration")
        ...
        with open(data_path, "r", encoding="utf-8") as f:
            for line in f:
                if line.strip():
                    ...
                    # Add answer if available (for validation set)
                    if "correct_answer" in data:
                        item["answer"] = data["correct_answer"]
                    elif "answer" in data:
                        item["answer"] = data["answer"]

                    dataset_list.append(item)

        logger.info(f"Loaded {len(dataset_list)} examples from {data_path}")
        return dataset_list
    ...
    def _get_prediction_with_trace(self, example: Dict) -> Tuple[Dict, str]:
        ...
        if question_type == "multi_choice":
            prompt = f"The following is a multiple choice question about medicine. Answer with only the letter (A, B, C, D).\n\nQuestion: {question}\n\nAnswer:"
        elif (
