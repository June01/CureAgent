...
from .gpu_utils import empty_cache, is_gpu_available, print_gpu_info
...
class TxAgent:
    ...
    def add_special_tools(self, tools, call_agent=False):
        if self.enable_finish:
            tools.append(
                self.tooluniverse.get_one_tool_by_one_name("Finish", return_prompt=True)
            )
            print("Finish tool is added")
        if call_agent:
            tools.append(
                self.tooluniverse.get_one_tool_by_one_name(
            ...
            for i in range(1, len(messages) + 1):
                if messages[-i]["role"] == "assistant":
                    ...
                    if len(assistant_messages) == 2:
                        break
            forbidden_ids = [
                tokenizer.encode(msg, add_special_tokens=False)
...
Generate **one summarized sentence** about "function calls' responses" with necessary information, and respond with a string:
    ...
    def function_result_summary(self, input_list, status, enable_summary):
        ...
        status["history"].append(
            self.summary_mode == "step"
            and status["summarized_step"]
        ...
        while idx < len(input_list):
            if (
                ...
                self.summary_mode == "length"
                and status["previous_length"] > self.summary_context_length
            ):
                ...
                if input_list[idx]["role"] == "assistant":
                    if "Tool_RAG" in str(input_list[idx]["tool_calls"]):
                        ...
                        last_call_idx = idx
                        this_thought_calls = (
                            input_list[idx]["content"] + input_list[idx]["tool_calls"]
                        )
                        function_response = ""

                elif (
                    input_list[idx]["role"] == "tool" and this_thought_calls is not None
                ):
            ...
            while next_round and current_round < max_round:
                ...
                if len(last_outputs) > 0:
                    (
                        ...
                        picked_tools_prompt,
                        special_tool_call,
                        current_gradio_history,
                    ) = yield from self.run_function_call_stream(
                        last_outputs,
                        return_message=True,
                        existing_tools_prompt=picked_tools_prompt,
                        message_for_call_agent=message,
                    ...
                    if special_tool_call == "Finish":
                        ...
                        or special_tool_call == "DirectResponse"
                    ):
                        history.append(
                ...
                if "[FinalAnswer]" in last_thought:
                    final_thought, final_answer = last_thought.split("[FinalAnswer]")
                    history.append(
